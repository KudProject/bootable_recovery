...
bool KeymasterOperation::updateCompletely(const std::string& input, std::string* output) {
    ...
    while (it != input.end()) {
        ...
        if (error != KM_ERROR_OK) {
            LOG(ERROR) << "update failed, code " << error << std::endl;
            mDevice = nullptr;
            return false;
        }
...
bool KeymasterOperation::finish() {
    ...
    mDevice = nullptr;
    if (error != KM_ERROR_OK) {
        LOG(ERROR) << "finish failed, code " << error << std::endl;
        return false;
    }
    return true;
}
...
Keymaster::Keymaster() {
    ...
    if (ret != 0) {
        LOG(ERROR) << "hw_get_module_by_class returned " << ret << std::endl;
        return;
    }
    if (module->module_api_version == KEYMASTER_MODULE_API_VERSION_1_0) {
        ...
        if (ret != 0) {
            LOG(ERROR) << "keymaster1_open returned " << ret << std::endl;
            return;
        }
        ...
        if (ret != 0) {
            LOG(ERROR) << "keymaster2_open returned " << ret << std::endl;
            return;
        }
        mDevice = std::make_shared<Keymaster2Device>(device);
    } else {
        LOG(ERROR) << "module_api_version is " << module->module_api_version << std::endl;
        return;
    }
}
...
/*bool Keymaster::generateKey(const keymaster::AuthorizationSet& inParams, std::string* key) {
    ...
    if (error != KM_ERROR_OK) {
        LOG(ERROR) << "generate_key failed, code " << error << std::endl;
        return false;
    }
...
bool Keymaster::deleteKey(const std::string& key) {
    ...
    if (error != KM_ERROR_OK) {
        LOG(ERROR) << "delete_key failed, code " << error << std::endl;
        return false;
    }
    return true;
}
...
KeymasterOperation Keymaster::begin(keymaster_purpose_t purpose, const std::string& key,
    ...
    if (error != KM_ERROR_OK) {
        LOG(ERROR) << "begin failed, code " << error << std::endl;
